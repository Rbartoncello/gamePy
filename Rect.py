...
class Rect:
    ...
    def move(self, x, y):
        if (self.rect.x + x > 0) and (self.rect.x + x + self.width / 2 - 25 < self.x_max): self.rect.x = self.rect.x + x
        if (self.rect.y + y > 0) and (
                self.rect.y + y + self.height / 2 - 25 < self.y_max): self.rect.y = self.rect.y + y

    def move_with_return(self, x, y):
        if self.rect.x + x + self.width / 2 - 25 >= self.x_max:
            self.rect.x = -1
        elif self.rect.y + y + self.height / 2 - 25 >= self.y_max:
            self.rect.y = -1
        else:
